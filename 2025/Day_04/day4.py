...
class part1:
    ...
    def _read_data(self):
        ...
        self.array = [[transform(j) for j in i] for i in d]

    def _find_answer(self):
        jd = jnp.array(self.array)

        kern = jnp.array([[1, 1, 1], [1, 0, 1], [1, 1, 1]])

        jd_convolve = jsp.signal.convolve2d(
            in1=jd, in2=kern, mode="same", boundary="fill", fillvalue=0
        )

        jd_is_removable = jnp.array((jd_convolve) < 4, dtype=jnp.float32) * jd
